...
def filter_words(words, skip_words):
    ...
    """
    # Define a new list that will hold new list without "unimportant" words.
    list_of_words = []
    # For every word in the list of words check if it is in the list of skip_words.
    for word in words:
        if not (word in skip_words):
            # If not add to the end of the new list.
...
def normalise_input(user_input):
    ...
    important_words_only = filter_words(no_spaces, skip_words)
    # Return new value.
    return important_words_only 